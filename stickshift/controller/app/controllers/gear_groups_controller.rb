class GearGroupsController < BaseController
  ...
  def update
    ...
    begin
      ...
      render_success(:ok, "gear_group", RestGearGroup.new(group_instance, gear_states, get_url, nolinks) , "UPDATE_GEAR_GROUP", "Patch gear group #{gear_group_id} for application '#{app_id}' with domain '#{domain_id}'")
    rescue Mongoid::Errors::DocumentNotFound
      return render_error(:not_found, "Gear group '#{gear_group_id}' not found for application #{app_id} on domain '#{domain_id}'", 101, "UPDATE_GEAR_GROUP")
    end    
  end
end
