...
class Application
  ...
  def calculate_gear_destroy_ops(ginst_id, gear_ids, additional_filesystem_gb)
    ...
    gear_ids.each do |gear_id|
      ...
      ops = [destroy_gear_op, unreserve_uid_op, delete_gear_op, track_usage_op]
      pending_ops.push *ops
      if additional_filesystem_gb != 0
      ...
      unless change[:from].nil?
        ...
        if change[:to].nil?
          ...
          if group_instance.addtl_fs_gb != change[:to_scale][:additional_filesystem_gb]
            ...
            group_instance.gears.each do |gear|
              fs_op = PendingAppOp.new(op_type: :set_gear_additional_filesystem_gb,
                ...
                saved_values: {"additional_filesystem_gb" => group_instance.addtl_fs_gb, "event" => event}, 
                prereq: [op._id.to_s])
          ...
          if scale_change < 0
            ...
            ops = calculate_gear_destroy_ops(group_instance._id.to_s, remove_ids, group_instance.addtl_fs_gb)
            pending_ops.push(*ops)
          end
        end
      end
