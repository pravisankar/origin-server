    ...
    while(pending_ops.where(:state => :completed).count > 0) do
      ...
      eligible_rollback_ops.each do|op|
        ...
        when :patch_user_env_vars
          ...
          result_io.append application.get_app_dns_gear.unset_user_env_vars(set_vars, application.get_gears_dns(true)) if set_vars.present?
          result_io.append application.get_app_dns_gear.set_user_env_vars(op.saved_values, application.get_gears_dns(true)) if op.saved_values.present?
        when :add_ssl_cert
          gear = get_gear_for_rollback(op)
      ...
      while(pending_ops.where(:state.ne => :completed).count > 0) do
        ...
        eligible_ops.each do|op|
          ...
          when :patch_user_env_vars
            ...
            result_io.append application.get_app_dns_gear.unset_user_env_vars(unset_vars, application.get_gears_dns(true)) if unset_vars.present?
            result_io.append application.get_app_dns_gear.set_user_env_vars(set_vars, application.get_gears_dns(true)) if set_vars.present? or op.args["push"]
